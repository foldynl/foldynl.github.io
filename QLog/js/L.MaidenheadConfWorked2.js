...
L.MaidenheadConfWorked = L.LayerGroup.extend({
	...
	redraw: function () {
		...
		// pokud wrap=false → omezíme rozsah na -180..180
		...
		for (var lon = left; lon < right; lon += (unit*2)) {
			// pokud wrap=false a lon mimo rozsah, přeskoč
			if (!this.options.wrap && (lon < -180 || lon >= 180))
				continue;
			...
			for (var lat = bottom; lat < top; lat += unit) {
				...
				} else if (grids_confirmed.some(e => regular.test(e))) {
	...
	_getLocator: function(lon, lat) {
		...
		// pokud wrap=true → přepočítej do -180..180, jinak ne
		if (this.options.wrap) {
			while (x < -180) {x += 360;}
			while (x > 180) {x -= 360;}
		}
